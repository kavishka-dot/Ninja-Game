...
class Animation:
    ...
    def copy(self):
        return Animation(self.images, self.img_duration, self.loop) # creates a copy. However, points to the same set of variables
    ...
    def update(self):
        if self.loop:
            ...
            if self.frame + 1 >= self.img_duration * len(self.images) - 1:
                self.done = True
